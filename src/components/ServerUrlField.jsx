...
export function ServerUrlField({ ns = "ISA", compact = false, dense = false, fixed = false }) {
  ...
  return (
    <Box sx={{ display: "inline-flex", alignItems: "center", gap: 0.75, minWidth: 0 }}>
      <ServerScopeSelect value={serverBase} onChange={setServerBase} scopes={scopes} ns={ns} compact={compact} dense={dense} fixed={fixed} />
      {!fixed && !dense && defaultBase && serverBase !== defaultBase ? (
